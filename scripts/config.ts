...
export function parseProviderConfig(commentBody: string): ProviderConfig {
  ...
  if (requestedModel) {
    ...
    if (modelLower.startsWith("gpt")) {
      if (!hasOpenAI)
        throw new Error(
          "Model appears to be an OpenAI model but OPENAI_API_KEY is not set.",
        );
      return { provider: "openai", model: requestedModel };
    }
    if (hasOpenRouter) {
  ...
  const defaultPriority: ProviderConfig["provider"][] = [
    "openrouter",
    "openai",
    "gemini",
    "anthropic",
  ...
  throw new Error(
    "No AI provider configured. Set OPENROUTER_API_KEY, OPENAI_API_KEY, GEMINI_API_KEY, and/or ANTHROPIC_API_KEY.",
  );
}
